...
const toRecord = (row: typeof productRatings.$inferSelect): IProductRatingRecord => ({
    id: row.id,
    userId: row.userId,
...
export class ProductRatingRepository implements IProductRatingRepository {
    ...
    async upsert(input: { userId: number; productId: number; rating: number }): Promise<IProductRatingRecord> {
